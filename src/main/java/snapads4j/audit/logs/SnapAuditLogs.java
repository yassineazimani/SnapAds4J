...
public class SnapAuditLogs implements SnapAuditLogsInterface {
	...
	this.endpointFetchByCampaign = this.apiUrl
		+ (String) fp.getProperties().get("api.url.audit.logs.by.campaign") + "?limit=50";
	this.endpointFetchByAd = this.apiUrl
		+ (String) fp.getProperties().get("api.url.audit.logs.by.ad") + "?limit=50";
	this.endpointFetchByAdSquad = this.apiUrl
		+ (String) fp.getProperties().get("api.url.audit.logs.by.adsquad") + "?limit=50";
	this.endpointFetchByCreative = this.apiUrl
		+ (String) fp.getProperties().get("api.url.audit.logs.by.creative") + "?limit=50";
