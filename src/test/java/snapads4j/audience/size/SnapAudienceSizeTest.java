...
public class SnapAudienceSizeTest {
	...
	Optional<AudienceSize> optSize = snapAudienceSize.getAudienceSizeBySquadSpec(oAuthAccessToken, adAccountId, adSquad);
	Assertions.assertThat(optSize.isPresent()).isTrue();
	optSize.ifPresent(s -> {
	...
	assertThatThrownBy(() -> snapAudienceSize.getAudienceSizeBySquadSpec(null, this.adAccountId, adSquad))
		.isInstanceOf(SnapOAuthAccessTokenException.class).hasMessage("The OAuthAccessToken must to be given");
	...
	assertThatThrownBy(() -> snapAudienceSize.getAudienceSizeBySquadSpec("", this.adAccountId, adSquad))
		.isInstanceOf(SnapOAuthAccessTokenException.class).hasMessage("The OAuthAccessToken must to be given");
	...
	assertThatThrownBy(() -> snapAudienceSize.getAudienceSizeBySquadSpec(oAuthAccessToken, this.adAccountId, null))
	...
	snapAudienceSize.getAudienceSizeBySquadSpec(this.oAuthAccessToken, this.adAccountId, adSquad);
	...
	Mockito.when(statusLine.getStatusCode()).thenReturn(400);
	Mockito.when(httpClient.execute(Mockito.any(HttpPost.class))).thenReturn(httpResponse);
	assertThatThrownBy(() -> snapAudienceSize.getAudienceSizeBySquadSpec(oAuthAccessToken, this.adAccountId, adSquad))
		.isInstanceOf(SnapResponseErrorException.class).hasMessage("Bad Request");
